...
fun NewWorkoutScreen(navController: NavController, newWorkoutViewModel: NewWorkoutViewModel = viewModel()) {
    Scaffold(
        ...
        val showExerciseMenu = remember { mutableStateOf(false) }

        Column(
            modifier = Modifier.padding(innerPadding).padding(16.dp)
        ) {
            Button(onClick = { showExerciseMenu.value = true }) {
                Text("Add Exercise")
            }
            DropdownMenu(
                expanded = showExerciseMenu.value,
                onDismissRequest = { showExerciseMenu.value = false }
            ) {
                DropdownMenuItem(text = {Text("Bench Press")}, onClick = { 
                    newWorkoutViewModel.addExercise("Bench Press")
                    showExerciseMenu.value = false
                })
                DropdownMenuItem(text = {Text("Squat")}, onClick = { 
                    newWorkoutViewModel.addExercise("Squat")
                    showExerciseMenu.value = false
                })
            }
